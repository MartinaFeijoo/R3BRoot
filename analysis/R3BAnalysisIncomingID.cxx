...
void R3BAnalysisIncomingID::Exec(Option_t* option)
{
    ...
    Double_t ToFraw_m1 = 0., PosCal_m1 = -10000.;
    ...
    {
      Int_t mult[fNumDet]; Int_t numDet;
      for (Int_t i=0; i<fNumDet; i++)
      {
        mult[i]=0; PosSci2_m1[i]=0.; TimeSci2_m1[i]=0.;
      }
        ...
        for (Int_t ihit = 0; ihit < nHits; ihit++)
        {
            ...
            if (mult[numDet]==0)
            {
              PosSci2_m1[numDet] = hittcal -> GetPosition();
              TimeSci2_m1[numDet] = hittcal -> GetTime();
              mult[numDet]++;
            }
    ...
    { Int_t mult[fNumDet]; Int_t numDet;
      for (Int_t i=0; i<fNumDet; i++) {mult[i]=0; PosSci2_m1[i]=0.; TimeSci2_m1[i]=0.;}
        ...
        for (Int_t ihit = 0; ihit < nHits; ihit++)
        {
            ...
            if (mult[numDet]==0)
            {
              timeLosV[numDet] = hittcal -> GetTime();
              mult[numDet]++;
            }
    ...
    for(int i=0; i<fNumDet;i++)
    {
      ...
      Velo_m1 = 1. / (fTof2InvV_p0->GetAt(i) + fTof2InvV_p1->GetAt(i) * (fToFoffset->GetAt(i) + ToFraw_m1)); // [m/ns]
      Beta_m1 = Velo_m1 / 0.299792458;
      Gamma_m1 = 1. / (TMath::Sqrt(1. - TMath::Power(Beta_m1, 2)));
      PosCal_m1 = PosSci2_m1[1];       // [mm] at S2
